...
; POKEMON CAUGHT WITH A ROCKET_BALL ARE NOT RECORDED IN THE POKEDEX

	ld a, [wIsInBattle]
	and a ; is this a wild mon caught in battle?
	jr nz, .copyEnemyMonDataR
...
.next4R
	push bc
	ld bc, 0 ; wPartyMon1DVs - wPartyMon1
	add hl, bc
	pop bc
	ld [hli], a
	ld [hl], b         ; write IVs
	ld bc, 0; (wPartyMon1HPExp - 1) - (wPartyMon1DVs + 1)
	add hl, bc
	ld a, 1
	ld c, a
	xor a
	ld b, a
	call CalcStat      ; calc HP stat (set cur Hp to max HP)
	ld a, 0 ;[H_MULTIPLICAND+1]
	ld [de], a
	inc de
	ld a, 0 ;[H_MULTIPLICAND+2]
	ld [de], a
	inc de
	xor a
	ld [de], a         ; box level
...
.copyEnemyMonDataR
	ld bc, wEnemyMon1DVs - wEnemyMon1
	add hl, bc
	ld a, 0; [wEnemyMonDVs] ; copy IVs from cur enemy mon
	ld [hli], a
	ld a, 0; [wEnemyMonDVs + 1]
	ld [hl], a
	ld a, 0; [wEnemyMonHP]    ; copy HP from cur enemy mon
	ld [de], a
	inc de
	ld a, 0; [wEnemyMonHP+1]
	ld [de], a
	inc de
	xor a
	ld [de], a                ; box level
	inc de
	ld a, 0; [wEnemyMonStatus]   ; copy status ailments from cur enemy mon
	ld [de], a
	inc de
.copyMonTypesAndMovesR
	ld hl, wMonHTypes
	ld a, [hli]  	; type 1
	ld [de], a
	inc de
...
;	predef WriteMonMoves
	pop de
	ld a, [wEnemyMonDVs]  ; set trainer ID to player ID
	inc de
	ld [de], a
	ld a, [wEnemyMonHP]
	inc de
...
;	callab CalcExperience
	...
	xor a
	ld b, NUM_STATS * 2
.writeEVsLoopR              ; set all EVs to 0
	ld a, $0
	inc de
	ld [de], a
	dec b
