...
; the base exp (which determines normal exp) is also halved
	...
	ld b, $7
;.halveExpDataLoop ; REMOVING THESE 5 LINES SHOULD... FIX/BUFF EXP ALL?
;	 srl [hl]
;	 inc hl
;	 dec b
...
; divide enemy speed by 4
	...
	ld a, [wNumRunAttempts]
	ld c, a
; add 47 to the quotient for each run attempt
.loop
	dec c
	jr z, .compareWithRandomValue
	ld b, 47
...
.pickMoveLoop
	...
	and a
	jr z, .pickMoveLoop
	cp $a4 + 1 ; max normal move number + 1 (this is Struggle's move number) ; Delete this and replace with below once moves are functional.
...
.recalculateStat ; recalculate affected stat
	...
	call Divide
	pop hl
; cap at 999 ; THEN AGAIN, MAYBE DON'T
;	ld a, [H_PRODUCT + 3]
;	sub 999 % $100
;	ld a, [H_PRODUCT + 2]
;	sbc 999 / $100
