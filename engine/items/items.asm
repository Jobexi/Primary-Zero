...
; The Rocket Ball always succeeds.
	cp ROCKET_BALL
	ld a, $1
	ld [wJobexi1], a
	jp z, .captured
...
.useRareCandy
	...
	ld a, [hl] ; a = level
	cp MAX_LEVEL
	jr z, .vitaminNoEffect ; can't raise level above 250
	cp 0
	jr z, .vitaminNoEffect ; can't raise level to 1 from Zero
	inc a
...
.RandomLoop
	call Random
	srl a
;	jr c, .SetBite ; DETERIMINES LIKELIHOOD OF A CATCH, BUT INTERFERES WITH RANDOMNESS
;	and %11
	cp 32 ; DETERMINES MAX RANDOM #
	jr nc, .RandomLoop
